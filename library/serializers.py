...
class CreateBorrowingTransactionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, validated_data):
        ...
        total_borrowed_books = sum(
            [
                transaction.books.filter(is_borrowed=True).count()
                for transaction in BorrowingTransaction.objects.filter(
                    borrower=borrower
                )
            ]
        )
        if total_borrowed_books >= 3:
            raise serializers.ValidationError(
                _("The user has already borrowed 3 books, can not borrow more books.")
            )
        ...
        if total_borrowed_books + len(books) > 3:
            raise serializers.ValidationError(
                _(
                    f"You cannot borrow {len(books)} more books as you already borrowed {total_borrowed_books} and you can not borrow more than 3 books."
                )
            )
        ...
        for book in books:
            if book.is_borrowed:
                raise serializers.ValidationError(
                    _(
                        f'The book "{book.book.title}" is currently being borrowed, choose another book or try again later.'
                    )
                )
        ...
        if due_date > max_return_date:
            raise serializers.ValidationError(
                _(
                    f"Return date cannot be more than {max_period_month} month(s) from today."
                )
            )
...
class ReturningTransactionSerializer(serializers.ModelSerializer):
    ...
    def get_borrower_books(self, borrower):
        borrower_books = {
            book: transaction.due_date
            for transaction in BorrowingTransaction.objects.filter(borrower=borrower)
    ...
    def validate_books(self, returned_books):
        ...
        for book in returned_books:
            if book not in borrower_books:
                raise serializers.ValidationError(
                    _(
                        f'The book "{book.book.title}" is not being borrowed by the user.'
                    )
                )
    ...
    def create(self, validated_data):
        ...
        borrower_books = self.get_borrower_books(borrower)
        total_penalty = sum(
            [self.get_book_penalty(book, borrower_books[book]) for book in books]
        )
